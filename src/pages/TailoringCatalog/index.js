    ...
    methods: {
        onChapterSelect: function (chapters) {
            ...
            if (chapters.length > 0) {
                ...
                this.breadcrumbs = [];
                this._chapterPath.forEach((node) =>
                    this.breadcrumbs.push({
                        text: node.number + " " + node.name,
                        disabled: true,
                    })
                );
            }
        ...
        onRequirementNewSave: function (text) {
            this.$axios
                .post(this.requirement._links.self.href, text, {
                    emulateJSON: true,
                })
                .then((response) => {
        ...
        onRequirementEditSave: function (text) {
            this.$axios
                .put(this.requirement._links.text.href, text, {
                    emulateJSON: true,
                })
                .then((response) =>
                    this.handleRequirementResponse(
                        this.requirement,
                        response.data
                    )
                )
        ...
        onRequirementSelect: function (requirement) {
            this.$axios
                .put(requirement._links.selected.href)
                .then((response) => {
                    this.handleRequirementResponse(requirement, response.body);
                })
                .catch((error) => {
                    this.isEdit = false;
                    console.log(error);
        ...
        onChapterRequirementsSelect: function (chapter, state) {
            if (this.chapter != null) {
                ...
                this.$axios
                    .put(
                        this.chapter._links.selection.href.replace(
                            "{selected}",
                            state
                        )
                    )
                    .then((response) => {
                        this.chapter.chapters = response.data.chapters;
                ...
                this.$confirm(this.$tc("chapter_selection.text"), {
            ...
            },
            {
                ...
                to: {
                    ...
                    params: {
                        ...
                        self: this.$route.params.previous,
                    },
                },
            },
