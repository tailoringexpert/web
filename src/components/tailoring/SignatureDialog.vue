...
<template>
    <Dialog :visible="active" :header="_signature.faculty" :modal="true" @update:visible="onClose">
        <template #footer>
            <Button :label="$t('close')" align-items-start @click="onClose" />
            <Button :label="$t('save')" align-items-end @click="onSave" />
        </template>

        <div class="card flex flex-col gap-4">
            <div class="flex flex-col gap-2">
                <label for="signee">{{ t('SignatureDialog.name') }}</label>
                <InputText id="signee" v-model="_signature.signee" type="text" />
            </div>
            <div class="flex flex-col gap-2">
                <label for="state">{{ t('SignatureDialog.signaturestate') }}</label>
                <InputText id="state" v-model="_signature.state" type="text" />
            </div>
            <div class="flex items-center">
                <Checkbox id="applicable" v-model="_signature.applicable" binary />
                <label for="applicable" class="ml-2">{{ t('SignatureDialog.applicable') }}</label>
            </div>
        </div>
    </Dialog>
</template>
